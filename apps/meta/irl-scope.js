(() => {
  ...
  function rowHtml(row,index) {
    ...
    const variants = expandable && open ? `<div class="current-variants">${row.variants.map(v=>`<button type="button" class="variant-row" data-explore-deck="${esc(v.name)}" data-explore-source="irl"><span>${esc(v.name)}</span><b>${pct(v.share*100)}</b><small>${v.entries} entries</small><span class="explore-arrow">›</span></button>`).join('')}</div>` : '';
  ...
  function render() {
    ...
    document.querySelectorAll('[data-irl-family]').forEach(row=>row.addEventListener('click',event=>{ if(event.target.closest('[data-explore-deck]')) return; const n=row.dataset.irlFamily; expanded.has(n)?expanded.delete(n):expanded.add(n); render(); }));
  }
