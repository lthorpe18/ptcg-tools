...
function mutate(label,fn){const state=readState();if(!state?.zones)return false;saveUndo(state);fn(state);log(state,label);writeState(state);location.reload();return true}
...
function moveCardComplete(cardId,destination,label){
  ...
  return mutate(label||`Moved ${card.name} to ${destination}`,state=>{
    ...
    if(field){
      ...
      if(row)row.attachedTo=null;
    }else if(from==='under'){
      if(row)row.stackedUnder=null;
    }
  });
}
...
function moveManagedCard(cardId,destination){
  const state=readState(),card=cardById(state,cardId);if(!card)return false;
  return moveCardComplete(cardId,destination,`${card.name} → ${destination==='lost'?'Lost Zone':destination}`);
}

function shuffleDeck(){
  const before=readState();if(!before?.zones?.deck||before.zones.deck.length<2)return false;
  return mutate('Shuffled deck',state=>{const deck=[...state.zones.deck];for(let i=deck.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[deck[i],deck[j]]=[deck[j],deck[i]]}state.zones.deck=deck});
}
function drawTop(){
  const before=readState();if(!before?.zones?.deck?.length)return false;
  const card=cardById(before,before.zones.deck[0]);
  return mutate(`Drew ${card?.name||'a card'}`,state=>{const id=state.zones.deck.shift();if(id)state.zones.hand.push(id)});
}
function clearDamage(cardId){
  const state=readState(),card=cardById(state,cardId);if(!card)return false;
  return mutate(`Cleared damage from ${card.name}`,s=>{const row=cardById(s,cardId);if(row)row.damage=0});
}
function changeDamage(cardId,delta){
  const state=readState(),card=cardById(state,cardId);if(!card)return false;
  return mutate(`${card.name}: damage ${delta>0?'+':''}${delta}`,s=>{const row=cardById(s,cardId);if(row)row.damage=Math.max(0,Number(row.damage||0)+delta)});
}
...
function currentSheetCardId(){
  const body=$('#sheetBody');if(!body)return null;
  const move=body.querySelector('[data-move-card]');if(move?.dataset?.moveCard)return move.dataset.moveCard;
  const title=$('#sheetTitle')?.textContent||'',state=readState();
  return state?.cards?.find(card=>card.name===title)?.id||null;
}
function zoneRowCopy(card){return `<div><strong>${esc(card.name)}</strong><small>${esc(cardMeta(card))}</small></div>`}

function enhanceSearchSheet(){
  if($('#sheet')?.hidden||$('#sheetTitle')?.textContent!=='Search Deck')return;
  const body=$('#sheetBody');if(!body||body.querySelector('.pt-deck-actions'))return;
  const actions=document.createElement('div');actions.className='pt-sheet-actions pt-deck-actions';
  actions.innerHTML='<button type="button" class="primary" data-pt-deck-action="shuffle">Shuffle deck</button><button type="button" data-pt-deck-action="draw">Draw top card</button>';
  body.prepend(actions);
}
function enhanceStadiumSheet(){
  if($('#sheet')?.hidden||$('#sheetTitle')?.textContent!=='Stadium')return;
  const state=readState(),id=state?.zones?.stadium?.[0],body=$('#sheetBody');if(!id||!body||body.querySelector('.pt-stadium-actions'))return;
  const actions=document.createElement('div');actions.className='pt-sheet-actions pt-stadium-actions';
  actions.innerHTML=`<button type="button" class="danger" data-pt-move-card="${esc(id)}" data-pt-destination="discard">Discard Stadium</button><button type="button" data-pt-move-card="${esc(id)}" data-pt-destination="hand">To hand</button>`;
  body.prepend(actions);
}
function enhanceCardSheet(){
  ...
  if(['active','bench'].includes(from)){
    ...
    if(existingFieldTitle){
      const grid=existingFieldTitle.nextElementSibling;
      if(grid&&!grid.querySelector('[data-pt-damage]')){
        const minus=document.createElement('button');minus.type='button';minus.dataset.ptDamage='-50';minus.dataset.ptCardId=cardId;minus.textContent='−50 damage';
        const clear=document.createElement('button');clear.type='button';clear.dataset.ptClearDamage=cardId;clear.textContent='Clear damage';
        const plus=document.createElement('button');plus.type='button';plus.dataset.ptDamage='50';plus.dataset.ptCardId=cardId;plus.textContent='+50 damage';
        grid.prepend(minus);grid.append(clear,plus);
      }
    }
  }

  const attached=attachedIds(state,cardId).map(id=>cardById(state,id)).filter(Boolean);
  const under=evolutionIds(state,cardId).map(id=>cardById(state,id)).filter(Boolean);
  const oldTitle=[...body.querySelectorAll('.sheet-section-title')].find(el=>el.textContent.trim()==='Under / attached');
  if(oldTitle){const oldList=oldTitle.nextElementSibling;oldTitle.remove();oldList?.remove()}

  if(attached.length){
    const title=document.createElement('div');title.className='sheet-section-title';title.textContent='Attached cards';
    const list=document.createElement('div');list.className='pt-manage-list';
    list.innerHTML=attached.map(row=>`<div class="pt-manage-row">${zoneRowCopy(row)}<div class="pt-manage-buttons"><button type="button" data-pt-move-card="${esc(row.id)}" data-pt-destination="hand">Hand</button><button type="button" class="danger" data-pt-move-card="${esc(row.id)}" data-pt-destination="discard">Discard</button><button type="button" data-pt-move-card="${esc(row.id)}" data-pt-destination="lost">Lost</button></div></div>`).join('');
    body.append(title,list);
  }
  if(under.length){
    const title=document.createElement('div');title.className='sheet-section-title';title.textContent='Evolution stack';
    const list=document.createElement('div');list.className='pt-manage-list';
    list.innerHTML=under.map(row=>`<div class="pt-manage-row">${zoneRowCopy(row)}<div class="pt-manage-buttons"><button type="button" data-card-id="${esc(row.id)}">View</button></div></div>`).join('');
    const note=document.createElement('p');note.className='pt-stack-note';note.textContent='Underlying Pokémon are kept with the Pokémon when it moves to Hand, Deck, Discard or Lost Zone.';
...
function refreshTargets(){
  const id=selectedId(),state=readState(),from=id?zoneOf(state,id):null;
  const deck=$('.side-pile[data-zone-button="deck"]');deck?.classList.toggle('pt-zone-target',!!id);
  const hand=$('#handTray');hand?.classList.toggle('pt-zone-target',!!id&&!['hand',null].includes(from));
}
...
function handleSelectedZoneClick(event){
  ...
  const stadium=event.target.closest('[data-zone-button="stadium"]');if(stadium){
    const state=readState(),card=cardById(state,id),from=zoneOf(state,id);
    if(from==='hand'&&card?.section==='trainers'){event.preventDefault();event.stopImmediatePropagation();return replaceStadium(id)}
  }
  const handTray=event.target.closest('#handTray');
  if(handTray&&!event.target.closest('.play-card,button')){event.preventDefault();event.stopImmediatePropagation();return moveCardComplete(id,'hand')}
  return false;
}
...
document.addEventListener('click',event=>{
  ...
  if(coreMove&&['hand','deck','discard','lost'].includes(coreMove.dataset.destination)){
    event.preventDefault();event.stopImmediatePropagation();moveCardComplete(coreMove.dataset.moveCard,coreMove.dataset.destination);return;
  }
  if(coreMove&&coreMove.dataset.destination==='stadium'){
    event.preventDefault();event.stopImmediatePropagation();replaceStadium(coreMove.dataset.moveCard);return;
  }
  if(handleSelectedZoneClick(event))return;
},true);

function start(){installStyles();ensureSetupTurnZero();enhanceSheets();refreshTargets();setInterval(()=>{ensureSetupTurnZero();enhanceSheets();refreshTargets()},250)}
