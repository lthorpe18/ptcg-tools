...
function hasUndo(){if(window.PTCGPlaytestCore)return false;try{return !!sessionStorage.getItem(UNDO_KEY)}catch{return false}}
...
function mutate(label,fn){const core=window.PTCGPlaytestCore;if(core?.mutate){core.mutate(label,fn);return}const state=readState();if(!state?.zones)return;saveUndo(state);fn(state);log(state,label);writeState(state);reload()}
...
function ensureTurnZeroSetup(){
  ...
  if(changed&&!window.PTCGPlaytestCore)writeState(state);
  return changed&&!window.PTCGPlaytestCore;
}
...
function moveSelectedToTop(){const selected=selectedHandCard();if(!selected)return false;const id=selected.dataset.cardId;mutate('Put a card from hand on top of deck',state=>{state.zones.hand=(state.zones.hand||[]).filter(cardId=>cardId!==id);state.zones.deck=[id,...(state.zones.deck||[]).filter(cardId=>cardId!==id)]});return true}
...
function evolutionStackIds(state,topId){const result=[];let parents=[topId];while(parents.length){const parent=parents.shift();const children=(state.zones.under||[]).map(id=>cardById(state,id)).filter(card=>card?.stackedUnder===parent);children.forEach(card=>{if(!result.includes(card.id)){result.push(card.id);parents.push(card.id)}})}return result}
function discardSelectedCard(){
  ...
  mutate(`Discarded ${card.name}`,state=>{const attached=(state.zones.attached||[]).map(attachedId=>cardById(state,attachedId)).filter(row=>row?.attachedTo===id).map(row=>row.id);const under=evolutionStackIds(state,id);const moving=[id,...attached,...under];moving.forEach(cardId=>{removeIdFromZones(state,cardId);const row=cardById(state,cardId);if(row){row.attachedTo=null;row.stackedUnder=null;row.rotated=false}state.zones.discard.push(cardId)})});return true
}
function startNextTurn(){const state=readState();if(!state?.zones)return;const current=Number(state.turn||0),nextTurn=current+1,canDraw=(state.zones.deck||[]).length>0;mutate(canDraw?`Started turn ${nextTurn} — drew 1 card`:`Started turn ${nextTurn} — deck empty`,s=>{s.turn=nextTurn;s.coin=null;s.turnFlowV2=true;if(canDraw)s.zones.hand.push(s.zones.deck.shift())})}
function toggleMarker(cardId,marker){const label=MARKERS.find(([key])=>key===marker)?.[1]||marker;mutate(`Toggled ${label}`,state=>{const card=cardById(state,cardId);if(!card)return;if(!Array.isArray(card.markers))card.markers=[];card.markers=card.markers.includes(marker)?card.markers.filter(value=>value!==marker):[...card.markers,marker]})}
...
function renderCardMarkers(){const state=readState();if(!state)return;$$('.play-card[data-card-id]').forEach(el=>{const card=cardById(state,el.dataset.cardId);const markers=Array.isArray(card?.markers)?card.markers:[];let tray=el.querySelector('.playtest-marker-tray');if(!markers.length){tray?.remove();return}const signature=markers.join('|');if(!tray){tray=document.createElement('span');tray.className='playtest-marker-tray';el.appendChild(tray)}if(tray.dataset.signature===signature)return;tray.dataset.signature=signature;tray.innerHTML=markers.map(marker=>`<span class="playtest-marker marker-${marker}" title="${markerLabel(marker)}">${marker==='ability'?'A':marker==='poisoned'?'P':marker==='burned'?'B':marker==='asleep'?'Z':marker==='confused'?'?':'!'}</span>`).join('')})}
function energyType(name){const value=String(name||'').replace(/^Basic\s+/i,'').replace(/\s+Energy$/i,'').trim();const known={Fire:['fire','F'],Water:['water','W'],Lightning:['lightning','L'],Grass:['grass','G'],Psychic:['psychic','P'],Fighting:['fighting','Fi'],Darkness:['darkness','D'],Metal:['metal','M'],Fairy:['fairy','Fa'],Colorless:['colorless','C']};return known[value]||['special',value.split(/\s+/).map(word=>word[0]).join('').slice(0,3).toUpperCase()||'E']}
function renderEnergyAttachments(){const state=readState();if(!state?.zones)return;const attached=(state.zones.attached||[]).map(id=>cardById(state,id)).filter(Boolean);$$('.play-card[data-card-id]').forEach(el=>{const targetId=el.dataset.cardId;const energies=attached.filter(card=>card.attachedTo===targetId&&card.section==='energy');let tray=el.querySelector('.playtest-energy-tray');if(!energies.length){tray?.remove();return}const groups=new Map();energies.forEach(card=>{const key=card.name||'Energy';groups.set(key,(groups.get(key)||0)+1)});const signature=[...groups.entries()].map(([name,count])=>`${name}:${count}`).join('|');if(!tray){tray=document.createElement('span');tray.className='playtest-energy-tray';el.appendChild(tray)}if(tray.dataset.signature===signature)return;tray.dataset.signature=signature;tray.innerHTML=[...groups.entries()].map(([name,count])=>{const [type,label]=energyType(name);return `<span class="playtest-energy-pill energy-${type}" title="${esc(name)}">${esc(label)}${count>1?` ×${count}`:''}</span>`}).join('')})}
function openPrizeViewer(){const state=readState();if(!state?.zones)return;const ids=[...(state.zones.prizes||[])],cards=ids.map(id=>cardById(state,id)).filter(Boolean);const sheet=$('#sheet'),body=$('#sheetBody');if(!sheet||!body)return;$('#sheetTitle').textContent='Prizes';$('#sheetEyebrow').textContent='ZONE';$('#sheetMeta').textContent=`${cards.length} remaining`;body.innerHTML=cards.length?`<p class="sheet-note">Your prize cards are shown below. Tap one to take it.</p><div class="prize-reveal-grid">${cards.map((card,index)=>`<button type="button" class="prize-reveal-card" data-enh-take-prize="${index}"><img src="${esc(imageUrl(card))}" alt="${esc(card.name)}" loading="eager" decoding="async"><span><strong>${esc(card.name)}</strong><small>${esc([card.set,card.number].filter(Boolean).join(' '))}</small><em>Take prize</em></span></button>`).join('')}</div>`:'<p class="sheet-note">No prizes remaining.</p>';sheet.hidden=false}
function takePrize(index){const state=readState();const id=state?.zones?.prizes?.[Number(index)];if(!id)return;const card=cardById(state,id);mutate(`Took prize${card?` — ${card.name}`:''}`,s=>{const prizeId=s.zones.prizes.splice(Number(index),1)[0];if(prizeId)s.zones.hand.push(prizeId)})}

function injectMarkerControls(){const sheet=$('#sheet');if(!sheet||sheet.hidden)return;const body=$('#sheetBody');if(!body||body.querySelector('.marker-controls'))return;const cardButton=body.querySelector('[data-move-card]')||body.querySelector('[data-card-id]');let cardId=cardButton?.dataset?.moveCard||cardButton?.dataset?.cardId||null;if(!cardId){const title=$('#sheetTitle')?.textContent||'';const state=readState();cardId=state?.cards?.find(card=>card.name===title)?.id||null}if(!cardId)return;const state=readState();const card=cardById(state,cardId);if(!card)return;const zone=Object.keys(state.zones||{}).find(key=>(state.zones[key]||[]).includes(cardId));if(!['active','bench'].includes(zone))return;const wrap=document.createElement('div');wrap.className='marker-controls';wrap.innerHTML=`<div class="sheet-section-title">Markers</div><div class="marker-grid">${MARKERS.map(([key,label])=>`<button type="button" data-marker="${key}" data-marker-card="${cardId}" class="${card.markers?.includes(key)?'active':''}">${label}</button>`).join('')}</div>`;body.appendChild(wrap)}
...
function refreshTurnUI(){const state=readState();if(!state)return;const turn=$('#turnNumber');if(turn)turn.textContent=String(Number(state.turn||0));const button=$('#endTurn');if(button)button.textContent=Number(state.turn||0)===0?'Start turn 1':'End turn'}
...
window.addEventListener('ptcg-playtest-render',refresh);
function startRefreshLoop(){installStyles();if(ensureTurnZeroSetup()){reload();return}refresh();window.setInterval(refresh,500)}
