...
function enhance(){
  ...
  const link=document.createElement('a');link.dataset.tournamentDayLink='true';link.className='primary-button full-button';link.href=`./tournament-day.html?participation=${encodeURIComponent(participation.id)}`;link.textContent=participation.tournamentDay?'Open Tournament Day':'Start Tournament';host.appendChild(link);
}
