(function(){
  ...
  function savedEvents(){const rows=(currentState().eventParticipations||[]).filter(p=>VALID_STATUSES.has(p.attendanceStatus)).map(participation=>({participation,event:snapshotToEvent(participation)}));const filtered=rows.filter(({participation,event})=>{const past=isPast(event),status=participation.attendanceStatus;if(planFilter==='attending')return status==='attending'&&!past;if(planFilter==='interested')return status==='interested'&&!past;if(planFilter==='history')return status==='attended'||status==='skipped'||past;return (status==='attending'||status==='interested')&&!past});filtered.sort((a,b)=>planFilter==='history'?sortDescending(a.event,b.event):sortAscending(a.event,b.event));return filtered.map(x=>x.event)}
  ...
  function cardHtml(event){
    const status=statusFor(event),link=externalLink(event),meta=detailLine(event),savedOrganiser=event.scope==='local'&&isSavedOrganiser(event),organiser=organiserName(event),statusHtml=status?`<span class="plan-pill ${status}">${escapeHtml(STATUS_LABELS[status])}</span>`:'',organiserHtml=savedOrganiser?'<span class="saved-venue-pill">★ Saved organiser</span>':'',attendLabel=status==='attending'?'Attending ✓':status?STATUS_LABELS[status]:`I'm attending`,attendClass=status==='attending'?' saved':'',saveButton=event.scope==='local'&&organiser?`<button class="venue-save-button${savedOrganiser?' saved':''}" type="button" data-action="organiser" aria-label="${savedOrganiser?'Remove saved organiser':'Save organiser'}">${savedOrganiser?'★':'☆'}</button>`:'',organiserRow=event.scope==='local'&&organiser?`<div class="event-venue-row event-organiser-row"><div class="event-organiser">${escapeHtml(organiser)}</div>${saveButton}</div>`:'',locationRow=event.scope==='local'?`<div class="event-location">${escapeHtml(placeLine(event))}</div>`:'';
  ...
  function render(){if(!dataset)return;renderLocation();renderYourOrganisers();const visible=visibleEvents();els.list.innerHTML=visible.map(cardHtml).join('');els.list.classList.toggle('hidden',visible.length===0);els.empty.classList.toggle('hidden',visible.length!==0);if(!visible.length)els.empty.textContent=activeView==='attending'?'No saved events match this view. Mark an event as attending or interested from Nearby or Majors.':savedOnly?'No upcoming events from your saved organisers match these filters.':'No events match these filters.';const title=activeView==='majors'?'Major events':activeView==='attending'?'Your events':savedOnly?'Saved organiser events':'Nearby events';els.resultsTitle.textContent=title;els.resultsCount.textContent=`${visible.length}`;renderSourceStatus()}
