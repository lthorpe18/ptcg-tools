...
async function renderSlot(){
  ...
  if(!slot){slot=document.createElement('button');slot.id='tournamentDeckSlot';slot.type='button';const idCalc=document.getElementById('openIdCalc');quick.insertBefore(slot,idCalc||quick.firstChild)}
  ...
  const host=slot.querySelector('.deck-slot-sprites');if(host&&urls.length)host.innerHTML=urls.map(url=>`<img src="${esc(url)}" alt="">`).join('');
}
...
async function openPicker(){
  ...
  const status=document.getElementById('deckPickerStatus'),controls=document.getElementById('deckPickerControls');
  status.textContent='Loading saved decks…';status.classList.remove('hidden');controls.classList.add('hidden');document.getElementById('saveOptionalDeck').disabled=true;document.getElementById('clearOptionalDeck').disabled=!playedRef();
  try{decks=(await window.PTCGDeckStore.all())||[]}catch(error){console.error('Tournament deck picker could not read decks',error);status.textContent=`Could not read saved decks${error?.message?`: ${error.message}`:''}`;return}
  ...
  renderVersions();
  status.textContent=decks.length?'':'No saved decks found.';status.classList.toggle('hidden',decks.length>0);controls.classList.remove('hidden');updateSaveState();
}
function renderVersions(){
  const deck=decks.find(item=>item.id===document.getElementById('optionalDeckSelect')?.value),select=document.getElementById('optionalVersionSelect');if(!select)return;
  const ref=suggestedRef();
  select.innerHTML=`<option value="">Choose exact version</option>${(deck?.versions||[]).slice().reverse().map(version=>`<option value="${esc(version.id)}">${esc(version.name?`${version.label} · ${version.name}`:version.label)}</option>`).join('')}`;
  const preferred=ref&&deck&&ref.deckId===deck.id?ref.deckVersionId:deck?.currentVersionId;
  if(preferred&&(deck?.versions||[]).some(version=>version.id===preferred))select.value=preferred;
  updateSaveState();
}
...
async function saveDeck(){const deck=decks.find(item=>item.id===document.getElementById('optionalDeckSelect')?.value),version=deck&&(deck.versions||[]).find(item=>item.id===document.getElementById('optionalVersionSelect')?.value);if(!deck||!version)return;let listHash=version.listHash||null;if(!listHash&&window.PTCGDeckParser?.hashDecklist)listHash=await window.PTCGDeckParser.hashDecklist(version.rawText||'');if(!listHash){alert('That deck version could not be identified.');return}const ref={deckId:deck.id,deckVersionId:version.id,listHash,deckNameSnapshot:deck.name||null,deckVersionLabelSnapshot:version.name?`${version.label} · ${version.name}`:version.label||null,archetypeSnapshot:deck.archetype||null,selectedAt:now()};window.PTCGStorage.updateParticipation(participationId,row=>{row.usedDeckRef=ref;row.tournamentDay={...(row.tournamentDay||{}),lastOpenedAt:now()};return row});applyRefToMatches(ref);closePicker();renderSlot()}
