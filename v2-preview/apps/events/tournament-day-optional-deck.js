...
async function loadDeckRows(){
  if(typeof window.PTCGDeckStore?.all!=='function')throw new Error('Deck store unavailable');
  try{return await window.PTCGDeckStore.all()}catch(primaryError){
    console.warn('Prepared deck read failed; trying raw deck read',primaryError);
    if(typeof window.PTCGDeckStore?.allRaw==='function')return await window.PTCGDeckStore.allRaw();
    throw primaryError;
  }
}
async function openPicker(){
  ...
  try{
    ...
    renderVersions();
    status.textContent=decks.length?'':'No saved decks found.';status.classList.toggle('hidden',decks.length>0);controls.classList.remove('hidden');save.disabled=decks.length===0;
  }catch(error){
    console.error('Tournament deck picker could not read decks',error);
    const detail=error?.message?`: ${error.message}`:'';
    status.textContent=`Could not load saved decks${detail}`;controls.classList.add('hidden');save.disabled=true;
  }
}
