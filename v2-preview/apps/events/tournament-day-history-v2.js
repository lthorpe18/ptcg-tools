...
function usedRef(){const row=participation();const ref=row?.usedDeckRef;return ref?.deckId?ref:null}
...
function enhanceDeckSummary(){const summary=$('deckSummary');if(!summary||summary.dataset.spriteEnhanced==='true')return;const ref=usedRef();if(!ref)return;const strong=summary.querySelector('strong');if(!strong)return;summary.dataset.spriteEnhanced='true';summary.classList.add('deck-summary-rich');const wrap=document.createElement('span');wrap.className='playing-deck-row';const sprites=document.createElement('span');sprites.className='playing-deck-sprites';sprites.setAttribute('aria-hidden','true');const text=document.createElement('span');text.className='playing-deck-text';text.textContent=strong.textContent;wrap.append(sprites,text);strong.replaceWith(wrap);spriteImgs(ref.archetypeSnapshot||ref.deckNameSnapshot||'',sprites)}
function enhanceRow(button){if(button.dataset.richHistory==='true')return;const match=window.PTCGMatchStore?.get?.(button.dataset.matchId);if(!match)return;button.dataset.richHistory='true';button.classList.add('rich-round-row');const isId=String(match.notes||'').trim().startsWith(ID_MARKER);const opponent=match.opponentArchetype||'Opponent';const sequence=gameSequence(match);button.innerHTML=`<span class="rich-round-number">${esc((match.roundLabel||'R').replace(/^Round\s*/i,'R'))}</span><span class="rich-matchup"><span class="rich-sprites rich-sprites-self" aria-hidden="true"></span><span class="rich-vs">vs</span><span class="rich-sprites rich-sprites-opponent" aria-hidden="true"></span><span class="rich-opponent">${esc(opponent)}</span></span><span class="rich-game-sequence ${esc(match.result)}">${esc(sequence)}</span><span class="rich-round-result ${esc(match.result)}${isId?' id':''}"><strong>${isId?'ID':resultLetter(match.result)}</strong></span>`;spriteImgs(usedArchetype(),button.querySelector('.rich-sprites-self'));spriteImgs(opponent,button.querySelector('.rich-sprites-opponent'))}
function enhance(){enhanceDeckSummary();document.querySelectorAll('#roundHistory [data-match-id]').forEach(enhanceRow)}
const history=$('roundHistory');if(history)new MutationObserver(()=>setTimeout(enhance,0)).observe(history,{childList:true,subtree:true});
const deck=$('deckSummary');if(deck)new MutationObserver(()=>{deck.dataset.spriteEnhanced='false';setTimeout(enhanceDeckSummary,0)}).observe(deck,{childList:true,subtree:true});
