...
function enhance(){
  document.querySelectorAll('.event-card').forEach(card=>{
    ...
    const link=document.createElement('a');link.dataset.tournamentLink='true';link.className='primary-link tournament-entry-link';link.href=`./tournament-day.html?build=20260903-2318&participation=${encodeURIComponent(participation.id)}`;link.textContent=active?'Tournament':needsCompletion?'Complete':'Start';
