(() => {
  ...
  function detailSource() {
    return $('deckDetail')?.querySelector('[data-detail-source].active')?.dataset.detailSource || 'online';
  }

  function syncDetail() {
    const detail = $('deckDetail');
    const host = $('deckDetailControlsHost');
    if (!detail || !host || detail.classList.contains('hidden')) return;
