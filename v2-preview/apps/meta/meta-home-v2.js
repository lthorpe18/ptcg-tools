(() => {
  ...
  function exactRows() {
    return (window.MetaData?.data?.(state.source)?.decks || [])
      ...
      .map(d => ({
        ...d,
        name: d.name,
        entries: Number(d.entries ?? d.players ?? 0),
        share: Number(d.share || 0),
        variants: [{ ...d, name:d.name, entries:Number(d.entries ?? d.players ?? 0), share:Number(d.share || 0) }],
      }))
  ...
  function rowHtml(row,index) {
    ...
    return `<article class="current-meta-row ${expandable?'expandable':''}" data-current-family="${esc(row.name)}">
      <div class="current-rank">${index+1}</div>
      <div class="current-name">${window.DeckSprites?.html?.(row.name,{size:34})||''}<span><b>${esc(row.name)}</b><small>${expandable?`${row.variants.length} variants · tap to ${open?'collapse':'expand'}`:`${Number(row.entries||0).toLocaleString()} entries`}</small></span></div>
      <div class="current-share"><b>${pct(row.share)}</b><small>${Number(row.entries||0).toLocaleString()} entries</small></div>
      <span class="row-chevron" aria-hidden="true">${expandable?(open?'⌃':'⌄'):''}</span>${variants}
    </article>`;
  }
  ...
  document.querySelectorAll('[data-current-source]').forEach(btn => btn.addEventListener('click', () => {
    state.source = btn.dataset.currentSource === 'irl' ? 'irl' : 'online';
    state.showAll = false;
