(() => {
  ...
  function render() {
    ...
    document.querySelectorAll('[data-irl-family]').forEach(row => row.addEventListener('click', () => { const name = row.dataset.irlFamily; if (expanded.has(name)) expanded.delete(name); else expanded.add(name); render(); }));
  }
  ...
  document.querySelectorAll('[data-current-source]').forEach(button => button.addEventListener('click', () => {
    ...
    if (source === 'irl') { irlScope = 'latest'; showAll = false; expanded.clear(); setTimeout(render, 0); }
    else { showAll = false; expanded.clear(); setTimeout(() => setOptions('online'), 0); }
  }));
  $('currentWindow')?.addEventListener('change', event => {
    if (irlActive()) { irlScope = event.currentTarget.value === 'all-irl' ? 'all-irl' : 'latest'; showAll = false; expanded.clear(); setTimeout(render, 0); }
