...
function renderSavedPicker(){const wrap=ensureSavedPicker();if(!wrap)return;const select=$('tmSavedPlayer');const names=activeUser?readNames():[];wrap.classList.toggle('hidden',!activeUser||!names.length);if(!select)return;const esc=value=>String(value).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');select.innerHTML='<option value="">Choose saved player…</option>'+names.map(name=>`<option value="${esc(name)}">${esc(name)}</option>`).join('')}
function seedVisiblePlayers(){if(!activeUser||seeding)return;const names=[...document.querySelectorAll('#tmPlayers .tm-player span,#tmRun [data-result="P1"],#tmRun [data-result="P2"]')].map(el=>el.textContent?.trim()).filter(Boolean);if(!names.length)return;const merged=cleanNames([...readNames(),...names]);if(JSON.stringify(merged)===JSON.stringify(readNames()))return;seeding=true;writeNames(merged);renderSavedPicker();seeding=false}
...
function updateSwissState(){const button=$('tmNextRound'),host=$('tmRun');if(!button||!host)return;const rounds=[...host.querySelectorAll('.tm-round')];if(!rounds.length)return;const latest=rounds[rounds.length-1];const latestStatus=latest.querySelector('.tm-round-head small')?.textContent?.trim();const latestNumber=Number((latest.querySelector('.tm-round-head b')?.textContent||'').match(/\d+/)?.[0]||0);const nativeNext=Number((button.textContent||'').match(/\d+/)?.[0]||latestNumber+1);
  if(button.disabled&&latestStatus!=='Complete'){button.textContent=`Complete Round ${latestNumber}`;return}
  if(button.disabled&&latestStatus==='Complete'&&nativeNext===latestNumber+1){button.textContent='Swiss complete';return}
  if(!button.disabled&&button.textContent.trim()==='Swiss complete')button.textContent=`Generate Round ${latestNumber+1}`;
}
function syncEnhancements(){positionNextRound();updateSwissState();seedVisiblePlayers();renderSavedPicker()}
function install(){
  ...
  refreshUser();window.addEventListener('ptcg:auth-change',refreshUser);
  if(window.PTCGCloud?.onAuthStateChange)window.PTCGCloud.onAuthStateChange(()=>refreshUser()).catch(()=>{});
  const observer=new MutationObserver(syncEnhancements);const workspace=$('tmWorkspace');if(workspace)observer.observe(workspace,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['disabled','class']});
