(function(){
  ...
  async function renderUser(){const user=await window.PTCGCloud.getUser();signedOut.hidden=!!user;signedIn.hidden=!user;if(user)$('userEmail').textContent=user.email||'Signed in';return user}
  async function action(fn,success){setBusy(true);message('Working…');try{const result=await fn();message(success(result),'good');await renderUser();return result}catch(err){message(err?.message||String(err),'error')}finally{setBusy(false)}}
  ...
  $('pullNow').addEventListener('click',()=>{if(!confirm('Restore the cloud snapshot onto this device? Existing local decks with different IDs are kept; matching local state may be replaced.'))return;action(()=>window.PTCGCloud.pull(),r=>{if(!r)return 'No cloud backup exists yet.';$('syncStatus').textContent=new Date(r.updated_at).toLocaleString();return 'Cloud data restored. Reload the app to see all restored state.'})});
